...
从寄存器到高速缓存到内存到外存，访问速度依次降低，但成本也依次降低。因此在计算机中为了控制成本和访问速度的平衡，容量是依次提高的。

3. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

将使用频率较低的数据存储在成本较低、访问速度较慢的存储介质中，将频率较高的少量数据存储在成本较高、速度较快的介质中，从而能够达到支持足够大的存储容量、成本低且整体访问速度较快的效果。

4. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

一般程序的体量并不大，很少遇到。但是如果内存泄漏严重的程序跑太多次会出现这个问题。解决方法是重启&修改程序……

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

将一个本来应该占用比较大内存的应用程序通过划分不同模块、将同一块内存空间分配给不同时调用的模块的方式减小同一个进程使用的内存。程序开发者需要给出模块间的调用关系。

2. 什么是交换技术？覆盖与交换有什么不同？

交换技术是在多个进程同时运行导致某个进程内存不够时，将某个其他进程暂停并整个放入外存中，使得当前进程有足够空间的技术。区别在于：覆盖是处理单个进程内部的空间不足，交换是处理进程间的内存空间冲突；覆盖处理的内存块较小，交换处理的块较大。
...
时间局部性：同一条指令/同一个数据的多次访问通常出现在较短的时间间隔里；空间局部性：一段时间内被连续访问的指令/数据通常被存在相邻的一小块空间里；分支局部性：对于一个分支，相邻时间内分支的结果通常相同。

2. 如何提高程序执行时的局部性特征？

例如二维数组循环时尽量按照存储顺序访问等，防止大量缺页发生。
...
虚拟存储将目前执行的命令放置到内存中，并监控内存中的指令和数据，将长时间未访问的存储到外存中。和交换的区别是以页/段为单位，而不是进程；与覆盖的区别是整个过程较为动态，不用预先分析程序块等。好处是更加灵活，挑战是操作系统需要进行监控和动态链接。

2. 虚拟存储需要什么样的支持技术？

硬件需要支持地址转换；操作系统要知道哪些内容在内存，哪些不在，并能做置换。
...
是只加载一部分页到内存中的页式存储。缺页中断处理需要将外存中的页加载到内存中并修改页表中的标志位。

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

页表项有一个标志位，表示这个页是否在内存；并有一个修改位，表示该页中是否有修改过；访问位，统计页是否被经常访问；保护位，该页允许的访问方式。

 2. 页式存储和虚拟页式存储的区别是什么？

页式存储是将一个进程需要的页都加载到内存中，只是存储不连续；虚拟页式存储除了不连续之外还存在有的内容不在内存中的问题，需要考虑置换的操作。

### 8.6 缺页异常

1. 缺页异常的处理流程？

首先到查找空闲页，如果没有的话找到最近访问最少的页面q，将q存储到外存（如果有修改）或丢弃（无修改），并修改标志位；然后用空出来的内存存储外存中读取到的缺失页，并修改此页表表项。

2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

设访问内存时间为n，外存时间为m，缺页率p，缺页中被修改的概率q，则时间
$$
t=n*(1-p)+m*p*(1+q)
$$
